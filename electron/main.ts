...
5. DESIGN AND SILHOUETTE LOCK — write this block in every prompt: the edit ONLY upgrades materials, fabric texture, lighting and photographic realism; ZERO changes to the design (graphic shapes, logo letterforms, print placement, scale, rotation, colors identical to the reference) and ZERO changes to the silhouette (same cut, proportions, sleeve length, collar height, hem line).
...
function composeEnhanceFallbackPrompt(view: string, notes: string): string {
  return (
    ...
    `DESIGN AND SILHOUETTE LOCK: this edit ONLY upgrades materials, fabric texture, lighting and photographic realism. ZERO changes to the design (graphic shapes, logo letterforms, print placement, scale, rotation, colors) and ZERO changes to the silhouette (same cut, proportions, sleeve length, collar height, hem line).\n\n` +
