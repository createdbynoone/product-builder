...
export function UpdateBar() {
  ...
  useEffect(() => {
    const cleanup = window.pb.onUpdateStatus((s) => setStatus(s as UpdateStatus))
    return cleanup
  }, [])
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            {phase === 'available' && (
              <>
                ...
                <span className="text-[11.7px] font-mono text-text-secondary tracking-wide">
                  Product Builder {version && <span className="text-text-primary">{version}</span>} disponible · iniciando descarga...
                </span>
                <Spinner />
              </>
            )}
            {phase === 'downloading' && (
              <>
                <span className="text-accent text-[11.7px]">↓</span>
                <span className="text-[11.7px] font-mono text-text-secondary tracking-wide">
                  Product Builder {version && <span className="text-text-primary">{version}</span>} · descargando
                </span>
            ...
            {phase === 'installing' && (
              <>
                <span className="text-accent text-[11.7px]">⚙</span>
                <span className="text-[11.7px] font-mono text-text-secondary tracking-wide">
                  Product Builder {version && <span className="text-text-primary">{version}</span>} · instalando...
                </span>
                <Spinner />
              </>
            )}
            {phase === 'ready' && (
              <>
                <span className="text-accent text-[11.7px]">✓</span>
                <span className="text-[11.7px] font-mono text-text-secondary tracking-wide">
                  Product Builder {version && <span className="text-text-primary">{version}</span>} instalada · reiniciando
                </span>
                <Spinner />
              </>
            )}
